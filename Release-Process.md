# JellyChain1 Release Process

This document contains the technical aspects of the JellyChain1 release process. The
intended audience is those who have been authorized by the JellyChain1 Technical
Committee (TC) to create, promote and sign official release builds for JellyChain1.

## Who can make releases?

Release authorization is given by the JellyChain1 TC. Once authorized, an individual
...
JellyChain1 team with Write permission level so they are able to tag the
release commit and push changes to the JellyChain1 GitHub repository
(see Steps 4 and 5).

### 2. npmjs.com release access

The individual making the release will need to be made an owner on the
`JellyChain1` package on npmjs.com so they are able to publish the release
(see Step 6).
...
- There is a branch in github used for the current major version of JellyChain1, named
  `JellyChain1`.
...
- For any given major version of JellyChain1 (current, previous or next) there is
...
- For any given major version of JellyChain1, there is a branch used for publishing
  releases.
- For the current major version of JellyChain1, the release branch is the
  "Master branch" named `JellyChain1`.
- For all other major versions of JellyChain1, the release branch is the
  "Version branch" named `<major-version>.x`.

"Proposal branch"

- A branch in git representing a proposed new release of JellyChain1. This can be a
...
most recent release of the major version of JellyChain1 that the release targets.
...
### Step 3. Update package.json to the new version number
